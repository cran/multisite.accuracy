.adapted_AROC.sp <-
function (formula.healthy, group, tag.healthy, data, est.surv.h = c("normal", 
    "empirical"), p = seq(0, 1, l = 101), B = 1000) 
{
    ...
    compute.ROC <- function(formula.healthy, group, tag.healthy, 
        data, est.surv.h, p = seq(0, 1, l = 101)) {
        ...
        pre.placement.values <- (data.d[, marker] - predict(fit0p, 
            newdata = data.d))/sigma0p
        ...
        aarocp <- .adapted_simpson(arocp, p)
        res <- list()
        res$p <- p
        res$ROC <- arocp
        res$AUC <- aarocp
    ...
    res.fit <- compute.ROC(formula.healthy = formula.healthy, 
        group = group, tag.healthy = tag.healthy, data = data, 
        est.surv.h = est.surv.h, p = p)
    arocp <- res.fit$ROC
    aarocp <- res.fit$AUC
    if (B > 0) {
        ...
        aarocpb <- numeric(B)
        for (l in 1:B) {
            data.boot.d <- res.fit$data.d[sample(nrow(res.fit$data.d), 
                replace = TRUE), ]
            ...
            res.boot <- compute.ROC(formula.healthy = formula.healthy, 
                group = group, tag.healthy = tag.healthy, data = data.boot, 
                est.surv.h = est.surv.h, p = p)
    ...
    poolROC <- matrix(0, ncol = length(columns), nrow = np, dimnames = list(1:np, 
        col.names))
    poolROC[, 1] <- arocp
    AUC <- aarocp
    if (B > 0) {
        poolROC[, 2] <- apply(arocpb, 1, .adapted_ql)
        poolROC[, 3] <- apply(arocpb, 1, .adapted_qh)
        AUC <- c(AUC, quantile(aarocpb, c(0.025000000000000001, 
            0.97499999999999998)))
    }
    names(AUC) <- col.names
    res <- list()
...
function (ROC, set.p) 
{
    ...
    integral <- (set.p[l.set.p] - set.p[1])/(l.set.p - 1)/3 * 
        (ROC[1] + ROC[l.set.p] + 4 * sum(ROC[seq(2, l.set.p - 
            1, by = 2)]) + 2 * sum(ROC[seq(3, l.set.p - 2, by = 2)]))
}
multisite.accuracy <-
    ...
    ...) 
{
    ...
    if (estimate == "auc") {
        auc = switch(site.method, covar = {
            m = .adapted_AROC.sp(formula.healthy = y.pred ~ site, 
                group = "y", tag.healthy = 0, data = data.frame(y, 
                  y.pred, site), ...)
            auc = unname(m$AUC[1])
            attr(auc, "method") = "adapted AROC.sp"
            auc
        }, meta = {
            yi = vi = c()
